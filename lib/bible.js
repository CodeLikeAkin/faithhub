/**
 * Bible verse text. KJV (the standard) is fetched live from the AO Lab Free Use
 * Bible API (bible.helloao.org) — no key, no rate limit. See CONTEXT.md rule #6 —
 * never swap KJV to a key-gated service. NLT is Tyndale-copyrighted and isn't on
 * that free API, so it's served from the local `bible_nlt` Supabase table instead
 * (bulk-loaded once via .claude/faithhub-pipeline/backfill-bible-nlt.js).
 *
 * Used by the Verse Explorer / key verses to show the text of a scripture the
 * pastor opened.
 */
...
// Display translation code -> AO Lab API translation id.
const AO_LAB_TRANSLATION_IDS = {
  KJV: "eng_kjv",
};

export const TRANSLATIONS = ["KJV", "NLT"];
...
export async function fetchPassage(
  ...
  const verses =
    translation === "NLT"
      ? await fetchNltVerses(id, chapter, start, end)
      : await fetchAoLabVerses(translation, id, chapter, start, end);
...
async function fetchAoLabVerses(translation, bookId, chapter, start, end) {
  const apiId = AO_LAB_TRANSLATION_IDS[translation] || AO_LAB_TRANSLATION_IDS.KJV;
  try {
    const res = await fetch(
      `https://bible.helloao.org/api/${apiId}/${bookId}/${chapter}.json`
    );
...
async function fetchNltVerses(bookId, chapter, start, end) {
  try {
    ...
    return data.map((r) => ({ number: r.verse, text: r.text }));
  } catch {
    return null;
  }
}
...
// Parse a free-text reference like "John 3:16" or "1 Corinthians 13:4-7" into
// { book, chapter, verseStart, verseEnd }. Returns null if it can't be parsed
// or the book isn't recognized. Used for word-study `reference` strings, which
// aren't stored as structured columns the way sermon_scriptures rows are.
export function parseReference(ref) {
  if (!ref) return null;
  const m = ref
    .trim()
    .match(/^((?:[123]\s*)?[A-Za-z][A-Za-z .]*?)\s+(\d+)\s*:\s*(\d+)(?:\s*[-–]\s*(\d+))?$/);
  if (!m) return null;
  const [, book, chapter, verseStart, verseEnd] = m;

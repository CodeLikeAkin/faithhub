...
/**
 * True if a Gemini SDK error is a quota/rate-limit rejection (HTTP 429 /
 * RESOURCE_EXHAUSTED) rather than some other failure (bad request, network).
 * Used to decide whether it's safe to fall back to Groq — a 400 means our
 * prompt is broken and Groq would fail the same way, but a 429 is purely a
 * Gemini-side capacity issue.
 */
export function isGeminiQuotaError(err) {
  const status = err?.status ?? err?.response?.status;
  return status === 429 || /RESOURCE_EXHAUSTED|"code":\s*429/i.test(String(err?.message || ""));
}

...
const isStreaming = (studies) =>
  studies.some((s) => s.blocks.some((b) => b.status === "searching" || b.status === "answering"));
...
function persist() {
  ...
  // Never save mid-stream: a half-written answer must not survive a reload.
  if (isStreaming(state.studies)) return;
  try {
